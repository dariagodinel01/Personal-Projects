...
def main():
    ...
    MACHINE_EVENTS_PATH = "gs://clusterdata_2019_h/machine_events-000000000000.parquet.gz"
    INSTANCE_EVENTS_PATH = [
        "gs://clusterdata_2019_h/instance_events-000000000105.parquet.gz",    
        "gs://clusterdata_2019_h/instance_events-000000000106.parquet.gz",    
        "gs://clusterdata_2019_h/instance_events-000000000107.parquet.gz",    
        "gs://clusterdata_2019_h/instance_events-000000000108.parquet.gz",    
        "gs://clusterdata_2019_h/instance_events-000000000109.parquet.gz"    
    ]
    INSTANCE_USAGE_PATH = [
        "gs://clusterdata_2019_h/instance_usage-000000000105.parquet.gz", 
        "gs://clusterdata_2019_h/instance_usage-000000000106.parquet.gz", 
        "gs://clusterdata_2019_h/instance_usage-000000000107.parquet.gz", 
        "gs://clusterdata_2019_h/instance_usage-000000000108.parquet.gz", 
        "gs://clusterdata_2019_h/instance_usage-000000000109.parquet.gz" 
    ] 

    print("Loading Dataframes...")

    # Map clumns to table heads form Schema
    machines_df = spark.read.parquet(MACHINE_EVENTS_PATH) \
        .select(
            col("time"),
            col("machine_id"),
            col("type").alias("event_type"),
            col("capacity.cpus").alias("CPUs")
        )
    tasks_df = spark.read.parquet(*INSTANCE_EVENTS_PATH) \
        .select(
            col("time"),
            col("collection_id").alias("job_id"),
            col("instance_index").alias("task_index"),
            col("machine_id"),
            col("type").alias("event_type"),
            col("scheduling_class"),
            col("resource_request.cpus").alias("cpu_request")
        )
    usage_df = spark.read.parquet(*INSTANCE_USAGE_PATH) \
        .select(
            col("collection_id").alias("job_id"),
            col("instance_index").alias("task_index"),
            col("machine_id"),
            col("average_usage.cpus").alias("cpu_rate")
        )
